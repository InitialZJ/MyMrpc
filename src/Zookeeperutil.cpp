...
// 全局的watcher函数，zkserver给zkclient的通知
...
void Zookeeperutil::start() {
  std::string host =
      MrpcApplication::GetInstance().getConfig().Load("zookeeperip");
  std::string port =
      MrpcApplication::GetInstance().getConfig().Load("zookeeperport");
  ...
  3. watcher回调线程
  */
  m_handle = zookeeper_init(connstr.c_str(), global_watcher, 30000, nullptr,
                            nullptr, 0);
  if (m_handle == nullptr) {
...
void Zookeeperutil::create(std::string path, std::string data, int state) {
  // 判断path是否存在，如果存在就不需要创建了
  // service结点下：方法名结点（存取数据ip:port）
...
std::string Zookeeperutil::getData(std::string path) {
  ...
  if (flag != ZOK) {
    std::cout << "get znod error" << std::endl;
    return "";
  } else {
